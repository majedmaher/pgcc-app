<div class="mt-100 w-100">
    <div class="card p-20">
        ...
        <h4>{{__('dashboard.page description', ['name'=> __('main.projects')])}}</h4>
        <div class="card m-10 p-50-100 md-p-20 shadow"> 
            ...
            <form class="w-100 d-flex flex-column gap-50 mt-50" method="POST" wire:submit.prevent='submit'>
                <div class="d-grid mt-50 grid-template-2 sm-grid-template-1 grid-gap-20">
                    <div class="grid-column-full">
                        <h4>{{__('dashboard.add item', ['name'=> __('dashboard.project')])}}</h4>
                    </div>
            ...
            <div class="save mt-50">
                <button class="mt-50" wire:click='addBtn'>{{__('dashboard.add item', ['name' => __('dashboard.project')])}}</button>
            </div>
            @endif
            ...
            <form class="w-100 d-flex flex-column gap-50 mt-50" method="POST" wire:submit.prevent='update'>
                ...
                <div class="d-grid grid-template-2 sm-grid-template-1 grid-gap-20">
                    <div class="grid-column-full">
                        <h4>{{__('dashboard.update item', ['name'=> __('dashboard.project')])}}</h4>
                    </div>
                    <div class="form-group">
                ...
                <table>
                  ...
                  @if(!empty($projects) && $projects->count())
                    @foreach($projects as $key => $project)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td><img src="{{ asset($project->image) }}" alt="{{$project->title}}" width="50" height="50"></td>
                            <td>{{ $project->title }}</td>
                            <td>{{ $project->description }}</td>
                            <td>
                                <div class="actions">
                                    <button wire:click='editBtn({{$project->id}})' class="btn btn-info">Edit</button>
                                    <button wire:click='deleteItem({{$project->id}})' wire:confirm='{{__('dashboard.confirmation message')}}' class="btn btn-danger">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @endforeach
